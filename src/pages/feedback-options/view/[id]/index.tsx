...
import { getFeedbackOptionById } from 'apiSdk/feedback-options';
import { FeedbackOptionInterface } from 'interfaces/feedback-option';
import { CustomerFeedbackListPage } from 'pages/customer-feedbacks';

function FeedbackOptionViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('feedback_option', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/feedback-options/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Option Text" text={data?.option_text} />
                ...
                {hasAccess('feedback_form', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Feedback Form"
                    text={
                      <Link as={NextLink} href={`/feedback-forms/view/${data?.feedback_form?.id}`}>
                        {data?.feedback_form?.id}
                      </Link>
                    }
                  />
                )}
